...
✅ when stack is implemented using array, we add/remove elements from the end of the array.Because this helps in least time complexity. Otherwise if we try to add elements from the begining then , every elements has to be shifted to its next index which is inefficient operation
✅ When stack is implmented using linkedlist, we add/remove array from the head of the linkedlist because it help us to avoid extra traversing to index to make it shift to the last. So the stack's top is head of the linked list and its bottom is the tail of the linked list.
